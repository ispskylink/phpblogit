...
function createPost($request_values)
	{
		...
		// validate form
		if (empty($title)) { array_push($errors, "Необходим заголовок."); }
		if (empty($body)) { array_push($errors, "Необходим текст для поста."); }
		if (empty($topic_id)) { array_push($errors, "Топик не выбран."); }
		// Get image name
	  	$featured_image = $_FILES['featured_image']['name'];
	  	if (empty($featured_image)) { array_push($errors, "Нужен фото файл на пост."); }
	  	// image file directory
	  	$target = basename($featured_image);
	  	if (!move_uploaded_file($_FILES['featured_image']['tmp_name'], $target)) {
	  		array_push($errors, "Невозможно загрузить фото. Уточните настроки для загрузки файлов на сервер.");
	  	}
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	function updatePost($request_values)
	{
		...
		if (!isset($request_values['featured_image'])) {
			// Get image name
			  $featured_image = $_FILES['featured_image']['name'];
		...
		if (count($errors) == 0) {
			...
			$_SESSION['message'] = "Пост не успешно не обновлен.";
			header('location: posts.php');
			exit(0);
		}
	}
	...
	if (isset($_GET['publish']) || isset($_GET['unpublish'])) {
		...
		togglePublishPost($post_id, $message, $pubparam);
	}
	// delete blog post
	function togglePublishPost($post_id, $message, $pubparam)
	{
		global $conn;
		$sql = "UPDATE posts SET published=$pubparam WHERE id=$post_id";
	...
	function getAllComments()
	{
		...
		foreach ($comments as $comment) {
			$comment['author'] = getPostAuthorById($comment['user_id']);
